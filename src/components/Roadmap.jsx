...
function Roadmap({ response }) {
  ...
  return (
    <ol className="space-y-1 print:grid print:grid-cols-2 print:gap-4">
      {roadmap.map((stepItem, index) => {
        ...
        return (
          <motion.li
            ...
            className="relative flex gap-4 pb-5 print:block print:pb-0"
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.35, ease: 'easeOut', delay: index * 0.08 }}
          >
            <div className="relative flex w-10 flex-col items-center pt-1 print:hidden">
              <span
            ...
            <motion.div whileHover={{ scale: 1.02 }} className="w-full print:w-auto">
              <Card
                className={`w-full rounded-xl border bg-[#161b22] p-4 print-card print:mb-0 print:shadow-none print:p-3 print:bg-[#f0f6fc] print:border-[#d0d7de] print:text-[#111]`}
              >
                <div className="flex items-start justify-between gap-3 print:block print:gap-0">
                  <div>
                    <p className="text-sm font-medium uppercase tracking-wide text-[#8b949e] print-label">
                      Step {stepItem.step}
                    </p>
                    <p className="mt-1 text-lg font-semibold text-[#f0f6fc] print-section-title">
                      {stepItem.skill}
                    </p>
                    {isCurrentStep ? (
                      <p className="mt-1 text-sm font-medium text-[#238636] print-label">Current step</p>
                    ) : null}
                  </div>
                  ...
                  <Button
                    ...
                    variant="secondary"
                    className="px-3 py-1.5 text-sm no-print"
                  >
                ...
                {expanded ? (
                  <div className="mt-3 rounded-lg border border-[#30363d] bg-[#161b22] p-3 print:bg-[#fff] print:border-[#d0d7de]">
                    <p className="text-sm font-medium text-[#f0f6fc] print-label">Reason</p>
                    <p className="mt-1 text-sm leading-relaxed text-[#8b949e] print-value">
                ...
                {stepItem.resources.length > 0 ? (
                  <div className="mt-3">
                    <p className="text-sm font-medium text-[#f0f6fc] print-label">Resources</p>
                    <ul className="mt-1 list-disc space-y-1 pl-5 text-sm text-[#8b949e] print-value">
                      ...
                      <li key={`${stepItem.skill}-${resourceIndex}`}>
                        {resource.url ? (
                          <a
                            ...
                            target="_blank"
                            rel="noreferrer"
                            className="text-[#58a6ff] underline hover:text-[#58a6ff] hover:underline transition-all duration-200 print:text-[#0969da]"
                          >
                            {resource.label}
                          </a>
                        ) : (
                  ...
                  <p className="mt-3 text-sm text-[#8b949e] print-value">No resources provided.</p>
                )}

                {!isLast ? <div className="mt-4 border-b border-[#21262c] print:hidden"></div> : null}
              </Card>
            </motion.div>
          </motion.li>
        )
